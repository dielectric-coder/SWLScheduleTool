...
- **azmap-gtk IPC** (`m` key): sends target coordinates and station details to a running azmap-gtk instance via a named pipe (FIFO) at `$XDG_RUNTIME_DIR/azmap-target.fifo`. If no azmap-gtk is running (FIFO open fails with ENXIO), launches a new instance via `subprocess.Popen` passing 2 positional args (transmitter site lat/lon as target), plus `-t` (station name) and `-d` (station detail string) flags. The center is deliberately **not** passed — azmap-gtk reads it from its own `~/.config/azmap.conf`, and passing the swl-sched QTH would override it. The `-d` flag passes `station|freq|country|site|lang|target` so azmap-gtk has full station info on first launch. FIFO wire format: `lat,lon,name|station|freq kHz|country|site|lang|target\n`. Subsequent `m` presses send updates via FIFO to the running instance.
